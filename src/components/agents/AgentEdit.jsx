...
const AgentEdit = ({ data, id }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await axios.post(`https://f6vfh6rc-2003.inc1.devtunnels.ms/dashboard/api/agent/edit/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`https://f6vfh6rc-2003.inc1.devtunnels.ms/dashboard/api/agent/edit/${id}`, formData);
            if (response.data.success) {
